...
import AdminIdeaCard from "@/components/admin-idea-card";
...
const fetchIdeas = async (
  category: string
): Promise<AdminIdeasWithUsersResponse> => {
...
export default function AdminIdeasPage() {
  ...
  const {
    ...
    queryKey: ["adminIdeasWithUsers", selectedCategory],
    queryFn: ({ queryKey }) => fetchIdeas(queryKey[1] as string),
  });

  function handleCategoryChange(category: string) {
    router.push(`/admin/ideas?category=${category}`);
  }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 overflow-y-auto flex-1">
              {isLoading
                ...
                : ideasWithUsers?.data.map((idea) => (
                    <AdminIdeaCard key={idea.idea.id} idea={idea} />
                  ))}
            </div>
          </div>
        </div>
